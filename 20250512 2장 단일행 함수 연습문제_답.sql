...
select name, tel, replace(tel,substr(tel,length(tel)-3,4),'****') "replace" from student
where deptno1=101;
...
SELECT * FROM PROFESSOR;
SELECT PROFNO, NAME, PAY, BONUS,TO_CHAR(PAY*12+NVL(BONUS,0),'999,999') "연봉" FROM PROFESSOR WHERE DEPTNO=201;
...
SELECT empno, ename, comm, NVL2(COMM,'Exist','null') FROM EMP WHERE DEPTNO=30;
...
SELECT DEPTNO, NAME, 
     DECODE(deptno,101,'Computer Engineering',
                     ...
                     103,'Software Engineering','ETC') FROM PROFESSOR;
...
select name, substr(birthday,4,2) "MONTH",
    case  when substr(birthday,4,2) in('01','02','03') then '1/4'
                              when substr(birthday,4,2) in('04','05','06') then '2/4'
                              when substr(birthday,4,2) in('07','08','09') then '3/4'
                              else '4/4' end "분기"
    from student;
...
select empno, ename, sal, 
     ...
     else 'Level 5' end "LEVEL(급여등급)" from emp;